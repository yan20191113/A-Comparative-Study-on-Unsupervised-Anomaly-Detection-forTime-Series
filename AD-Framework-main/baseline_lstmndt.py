...
from utils.outputs import LSTMNDTOutput
...
class Model(nn.Module):
    def __init__(self, x_dim, h_dim, n_prediction, dropout=0.3):
        ...
        # units
        self.model_layer1 = nn.LSTM(x_dim, h_dim, batch_first=True)
        self.model_layer2 = nn.Dropout(dropout)
        self.model_layer3 = nn.LSTM(h_dim, h_dim, batch_first=True)
        self.model_layer4 = nn.Dropout(dropout)
        self.model_layer5 = nn.Linear(h_dim, n_prediction)

    def forward(self, input):
        layer1_output = self.model_layer1(input)[0]
        layer2_output = self.model_layer2(layer1_output)
        layer3_output = self.model_layer3(layer2_output)[0][:,-1]
        layer4_output = self.model_layer4(layer3_output)
        layer5_output = self.model_layer5(layer4_output)
        return layer5_output


class LSTMNDT(nn.Module):
    def __init__(self, file_name, config):
        super(LSTMNDT, self).__init__()
        # file info
        ...
        if self.save_model:
            ...
            self.save_model_path = \
                './save_model/{}/LSTMNDT_hdim_{}_rollingsize_{}' \
        ...
        # units
        self.model = Model(x_dim=self.x_dim, h_dim=self.h_dim, n_prediction=1, dropout=0.3)
    ...
    def fit(self, train_input, train_label, valid_input, valid_label, test_input, test_label, abnormal_data, abnormal_label, original_x_dim):
        ...
        if self.load_model == True and self.continue_training == False:
            ...
            for epoch in range(self.epochs):
                ...
                for i, (batch_x, _) in enumerate(train_data):
                    # input the history values to predict the first dim of the last step
                    opt.zero_grad()
                    batch_x = batch_x.to(device)
                    batch_x, batch_y = batch_x[:,:-1,:], batch_x[:,-1,0]
                    batch_x_prediction = self.forward(batch_x).squeeze()
                    batch_loss = loss_fn(batch_x_prediction, batch_y)
                    batch_loss.backward()
                ...
                with torch.no_grad():
                    for i, (val_batch_x, _) in enumerate(valid_data):
                        ...
                        val_batch_x, val_batch_y = val_batch_x[:, :-1, :], val_batch_x[:, -1, 0]
                        val_batch_x_prediction = self.forward(val_batch_x).squeeze()
                        val_batch_loss = loss_fn(val_batch_x_prediction, val_batch_y)
        ...
        with torch.no_grad():
            ...
            for i, (batch_x, _) in enumerate(test_data):
                ...
                batch_x, batch_y = batch_x[:, :-1, :], batch_x[:, -1, 0]
                batch_x_prediction = self.forward(batch_x).squeeze()
                batch_x_[:,-1,0] = batch_x_prediction
                cat_xs.append(batch_x_)

            cat_xs = torch.cat(cat_xs)
            lstmndt_output = LSTMNDTOutput(dec_means=cat_xs, best_TN=None, best_FP=None, best_FN=None, best_TP=None,
...
def RunModel(train_filename, test_filename, label_filename, config, ratio):
    ...
    model = LSTMNDT(file_name=train_filename, config=config)
    ...
    if config.save_output:
        ...
        np.save('./outputs/NPY/{}/Dec_LSTMNDT_hdim_{}_rollingsize_{}_{}_pid={}.npy'.format(config.dataset, config.h_dim, config.rolling_size, train_filename.stem, config.pid),
                dec_mean_unroll)
    ...
    if config.save_figure:
        if original_x_dim == 1:
            ...
            plt.tight_layout()
            # plt.show()
            plt.savefig('./figures/{}/Ori_LSTMNDT_hdim_{}_rollingsize_{}_{}_pid={}.png'.format(config.dataset, config.h_dim, config.rolling_size, train_filename.stem, config.pid),
                        dpi=600)
            plt.close()
            ...
            plt.tight_layout()
            # plt.show()
            plt.savefig('./figures/{}/Dec_LSTMNDT_hdim_{}_rollingsize_{}_{}_pid={}.png'.format(config.dataset, config.h_dim, config.rolling_size, train_filename.stem, config.pid),
                        dpi=600)
            plt.close()
            ...
            # plt.show()
            plt.savefig(
                './figures/{}/VisOut_LSTMNDT_SD_hdim_{}_rollingsize_{}_{}_pid={}.png'.format(config.dataset, config.h_dim, config.rolling_size, train_filename.stem, config.pid),
                dpi=300)
            plt.close()
            ...
            # plt.show()
            plt.savefig(
                './figures/{}/VisOut_LSTMNDT_hdim_{}_rollingsize_{}_MAD_{}_pid={}.png'.format(config.dataset, config.h_dim, config.rolling_size, train_filename.stem, config.pid),
                dpi=300)
            plt.close()
            ...
            # plt.show()
            plt.savefig(
                './figures/{}/VisOut_LSTMNDT_IQR_hdim_{}_rollingsize_{}_{}_pid={}.png'.format(config.dataset, config.h_dim, config.rolling_size, train_filename.stem, config.pid),
                dpi=300)
            plt.close()
        else:
    ...
    if config.use_spot:
        ...
        metrics_result = MetricsResult(TN=TN, FP=FP, FN=FN, TP=TP, precision=precision,
                                       ...
                                       best_roc_auc=rae_output.best_roc_auc, best_cks=rae_output.best_cks, min_valid_loss=rae_output.min_valid_loss)
    ...
    #for registered_dataset in ["Credit"]:
        ...
        if args.load_config:
            config = RAEConfig(dataset=None, x_dim=None, h_dim=None, preprocessing=None, use_overlapping=None,
                               ...
                               use_last_point=None, save_config=None, load_config=None, server_run=None,
                               robustness=None,
                               pid=None, save_results=None)
            try:
                config.import_config('./config/{}/Config_LSTMNDT_hdim_{}_rollingsize_{}_pid={}.json'.format(config.dataset, config.h_dim, config.rolling_size, config.pid))
            except:
                print('There is no config.')
        else:
            config = RAEConfig(dataset=args.dataset, x_dim=args.x_dim, h_dim=args.h_dim,
        ...
        if args.save_config:
            ...
            config.export_config(
                './config/{}/Config_LSTMNDT_hdim_{}_rollingsize_{}_pid={}.json'.format(config.dataset, config.h_dim,
        ...
        for train_file in train_path.iterdir():
            ...
            if config.save_results == True:
                ...
                result_dataframe.to_csv(
                    './results/{}/Results_LSTMNDT_hdim_{}_rollingsize_{}_{}_pid={}.csv'.format(config.dataset, config.h_dim, config.rolling_size, train_file.stem, config.pid),
                    index=False)
