...
class CAE(nn.Module):
    def __init__(self, file_name, config, gpu_id):
        ...
        if self.save_model:
            ...
            self.save_model_path = \
                './save_model/{}/2DCNN' \
                '_hdim_{}_rollingsize_{}_{}_pid={}.pt'.format(self.dataset, self.h_dim, self.rolling_size, Path(self.file_name).stem, self.pid)
        else:
        ...
        self.autoencoder = nn.Sequential(
            Conv2d_Same(1, self.h_dim, kernel_size=(3,3)),
            nn.ReLU(),
            Conv2d_Same(self.h_dim, int(self.h_dim/2), kernel_size=(3,3)),
            nn.ReLU(),
            Conv2d_Same(int(self.h_dim/2), int(self.h_dim/4), kernel_size=(3,3)),
            nn.ReLU(),
            Conv2d_Same(int(self.h_dim/4), int(self.h_dim/4), kernel_size=(3,3)),
            nn.ReLU(),
            Conv2d_Same(int(self.h_dim/4), int(self.h_dim/2), kernel_size=(3,3)),
            nn.ReLU(),
            Conv2d_Same(int(self.h_dim/2), self.h_dim, kernel_size=(3,3)),
            nn.ReLU(),
            Conv2d_Same(self.h_dim, 1, kernel_size=(3, 3)),
            nn.Sigmoid()
        )
    ...
    def fit(self, train_input, train_label, valid_input, valid_label, test_input, test_label, abnormal_data, abnormal_label, original_x_dim):
        ...
        valid_input = np.expand_dims(valid_input, 1)
        ...
        memory_usage_nvidia = np.mean(epoch_memory_usage)
        ...
        with torch.no_grad():
            ...
            cat_xs = torch.cat(cat_xs).unsqueeze(1)
...
def RunModel(train_filename, test_filename, label_filename, config, ratio, gpu_id):
    ...
    model = CAE(file_name=train_filename, config=config, gpu_id=gpu_id)
    model.to(device)
    cae_output = None
    ...
    if config.save_output:
        ...
        np.save('./outputs/NPY/{}/Dec_2DCNN_{}_pid={}.npy'.format(config.dataset, Path(train_filename).stem, config.pid), dec_mean_unroll)
    ...
    if config.use_spot:
        ...
        metrics_result = MetricsResult(TN=TN, FP=FP, FN=FN, TP=TP, precision=precision,
                                       ...
                                       min_valid_loss=cae_output.min_valid_loss, testing_time=cae_output.testing_time,
                                       training_time=cae_output.training_time, total_params=None,
                                       estimated_total_size=None, memory_usage_nvidia=cae_output.memory_usage_nvidia)
        return metrics_result
...
if __name__ == '__main__':
    ...
    #for registered_dataset in ["MSL", "NAB"]:
        ...
        if args.load_config:
            ...
            try:
                config.import_config('./config/{}/Config_2DCNN_pid={}.json'.format(config.dataset, config.pid))
            except:
        ...
        if args.save_config:
            ...
            config.export_config('./config/{}/Config_2DCNN_pid={}.json'.format(config.dataset, config.pid))
        # %%
        ...
        train_logger, file_logger, meta_logger = create_logger(dataset=args.dataset,
                                                               ...
                                                               train_logger_name='2DCNN_train_logger',
                                                               file_logger_name='2DCNN_file_logger',
                                                               meta_logger_name='2DCNN_meta_logger',
                                                               model_name='2DCNN',
                                                               pid=args.pid)
        ...
        for train_file in train_path.iterdir():
            ...
            metrics_result = RunModel(train_filename=train_file, test_filename=test_file, label_filename=label_file,
                                      config=config, ratio=args.ratio, gpu_id=gpu_id)
            ...
            if config.save_results == True:
                ...
                result_dataframe.to_csv(
                    './results/{}/Results_2DCNN_hdim_{}_rollingsize_{}_{}_pid={}.csv'.format(config.dataset, config.h_dim,
                                                                                           config.rolling_size,
